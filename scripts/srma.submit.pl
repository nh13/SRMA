...
#   - documentation: range can be a chromosome name or chr1:1-100 etc.
		...
		for(my $i=0;$i<scalar(@genomeInfo);$i++) {
			...
			for(my $start=1;$start <= $chrSize;$start+=$splitSize) {
				...
				if(1 == $within_range) {
					...
					$cmd .= " QUIET=true";
					$cmd .= " VALIDATION_STRINGENCY=".$data->{'samOptions'}->{'validationStringency'} if(defined($data->{'srmaOptions'}->{'validationStringency'}));
